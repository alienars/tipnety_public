...
const sortWrapper = document.getElementById('sort-dropdown-wrapper');
  ...
  // اطمینان از وجود المنت‌ها قبل از افزودن event listener
  if (sortButton && sortMenu && sortWrapper && sortLabel) {
    ...
    // مدیریت انتخاب گزینه‌ها از منو
    sortMenu.addEventListener('click', (event) => {
      const targetLink = event.target.closest('a');
      if (targetLink && targetLink.dataset.sort) {
        event.preventDefault(); // جلوگیری از رفرش صفحه
        
        // به‌روزرسانی متن دکمه اصلی
        sortLabel.textContent = targetLink.dataset.sort;
        
        // بستن منو
        sortMenu.classList.add('hidden');
        
        // در اینجا می‌توانید منطق اصلی مرتب‌سازی را فراخوانی کنید
        // console.log('Sorting by:', targetLink.dataset.sort);
      }
    });
  }
